...
export default async function CityPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  let items: any[] = [];
  try {
    const data = await listPublishedAdsFromDb({ city: slug, page: 1, pageSize: 12 });
    items = data.items;
  } catch {}
  return (
      ...
      <main className="mx-auto w-full max-w-6xl px-4 py-10">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-semibold tracking-tight">
              City: <span className="capitalize">{slug.replaceAll("-", " ")}</span>
            </h1>
            <p className="mt-2 text-sm text-muted-foreground">
              Showing active-only ads in this city.
            </p>
          </div>
        ...
        {items.length === 0 ? (
          <Card className="mt-8 p-8 text-center">
            <p className="font-semibold">No listings in this city yet</p>
            <p className="mt-1 text-sm text-muted-foreground">
              Be first to publish in this city. Until then, browse all active ads.
            </p>
